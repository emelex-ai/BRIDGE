...
def update_multi_tables(c, epoch, data_single):
    ...
    if c.wandb:
        # A hack. I do not know how to deal with wandb.plot.line in the wandb wrapper
        ...
        real_wandb.log({"epoch_random_multi": table_multi})
        ...
        histogram = real_wandb.plot.histogram(
            table_multi, value="loss", title="my Histogram"
        )

        # Log custom tables, which will show up as custommizable charts in the W&B UI
        real_wandb.log({"line_1": line_plot, "histogram_1": histogram})
