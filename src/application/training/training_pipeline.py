...
class TrainingPipeline(ABC):
    def __init__(self, model, config, dataset):
        ...
        self.model = model
        self.opt = pt.optim.AdamW(model.parameters(), config.learning_rate)
        self.device = pt.device(config.device)
        self.model.to(self.device)
        self.epochs_completed = 0
        self.train_dataset_slices, self.val_dataset_slices = self.create_data_slices()
    ...
    def create_data_slices(self):
        ...
        train_slices = [
            slice(i, min(i + self.config.batch_size_train, cutpoint))
            for i in range(0, int(cutpoint), self.config.batch_size_train)
        ]
        val_slices = [
            slice(i, min(i + self.config.batch_size_val, len(self.dataset)))
            for i in range(int(cutpoint), len(self.dataset), self.config.batch_size_val)
        ]
    ...
    def train_single_epoch(self, epoch):
        ...
        total_loss = 0
        example_ct = 0
        for step, batch_slice in enumerate(self.train_dataset_slices):
        ...
        avg_loss = total_loss / len(self.train_dataset_slices)
    ...
    def validate_single_epoch(self, epoch):
        ...
        example_ct = 0
        with torch.no_grad():
            for step, batch_slice in enumerate(self.val_dataset_slices):
                batch = self.dataset[batch_slice]
                logits = self.forward(batch)
                loss = self.compute_loss(logits, batch)
        ...
        avg_loss = total_loss / len(self.val_dataset_slices)
    ...
    def save_model(self, epoch):
        if epoch % self.config.save_every == 0:
            model_path = f"{self.config.model_path}/model_epoch_{epoch}.pth"
