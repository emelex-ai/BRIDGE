...
class Model(nn.Module):
    ...
    def forward_op2op(
        ...
        orthography_encoding = self.embed_orth_tokens(orth_enc_input)
        phonology_encoding = self.embed_phon_tokens(phon_enc_input)

        gp_encoding = (
            self.gp_multihead_attention(
                orthography_encoding, phonology_encoding, phonology_encoding, key_padding_mask=phon_enc_pad_mask
            )[0]
        ...
        pg_encoding = (
            self.pg_multihead_attention(
                phonology_encoding, orthography_encoding, orthography_encoding, key_padding_mask=orth_enc_pad_mask
            )[0]
            + phonology_encoding
        )
        ...
        global_embedding = self.global_embedding.expand(gp_pg.shape[0], -1, -1)
        gp_pg = torch.cat((global_embedding, gp_pg), dim=1)
        gp_pg_padding_mask = torch.cat(
            (torch.zeros((gp_pg.shape[0], self.d_embedding), device=self.device, dtype=torch.bool), gp_pg_padding_mask),
            dim=-1,
        )
        ...
        phon_output = self.phonology_decoder(
            phon_dec_input, memory=final_encoding, tgt_key_padding_mask=phon_dec_pad_mask
        )

        orth_token_logits = self.linear_orthography_decoder(orth_output).transpose(1, 2)
        phon_token_logits = self.linear_phonology_decoder(phon_output).view(-1, 2, -1).transpose(1, 2)
    ...
    def embed_phon_tokens(self, tokens):
        # tokens: list of lists of tensors (batch_size, phoneme_sequence_length, feature_indices)
        batch_size = len(tokens)
        padding_value = 0  # Define your padding index

        # Get the maximum phoneme sequence length and feature length
        max_phoneme_len = max(len(batch) for batch in tokens)
        max_feature_len = max(toke.numel() for batch in tokens for toke in batch)

        # Prepare a 3D tensor to hold the padded tokens
        tokes_tensor = torch.full(
            (batch_size, max_phoneme_len, max_feature_len),
            padding_value,
            dtype=torch.long,
            device=self.device,
        )

        # Fill the tensor with token indices
        for i, batch in enumerate(tokens):
            for j, tokes in enumerate(batch):
                length = tokes.numel()
                tokes_tensor[i, j, :length] = tokes.to(self.device)

        # Perform embedding lookup
        embeddings = self.phonology_embedding(tokes_tensor)  # Shape: (B, P, F, D)

        # Create a mask for valid tokens (non-padding)
        mask = (tokes_tensor != padding_value).unsqueeze(-1)  # Shape: (B, P, F, 1)

        # Compute the average embeddings
        embeddings = embeddings * mask
        sum_embeddings = embeddings.sum(dim=2)  # Sum over feature dimension
        counts = mask.sum(dim=2).clamp(min=1)  # Avoid division by zero
        avg_embeddings = sum_embeddings / counts  # Shape: (B, P, D)

        # Add positional embeddings
        position_ids = torch.arange(max_phoneme_len, device=self.device).unsqueeze(0).expand(batch_size, -1)
        position_embeddings = self.phon_position_embedding(position_ids)
        output_embedding = avg_embeddings + position_embeddings
