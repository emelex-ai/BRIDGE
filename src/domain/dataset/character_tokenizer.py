...
class CharacterTokenizer:
    def __init__(self, list_of_characters: List[str]):
        # Initialize vocabulary with special tokens
        self.vocab = ["[BOS]", "[EOS]", "[CLS]", "[UNK]", "[PAD]"] + list_of_characters
    ...
    def decode(self, list_of_ints: List[List[int]]) -> List[str]:
        try:
            decoded_strings = [
                "".join([self.idx_2_char[i] for i in ints if self.idx_2_char[i] not in ["[PAD]", "[BOS]", "[EOS]"]])
