...
class TestNonwordGenerate:
    ...
    def test_multiple_nonwords_batch(self, MockModel, bridge_tokenizer):
        ...
        mock_model.generate.return_value = MagicMock(
            spec=GenerationOutput,
            phon_tokens=[[torch.tensor([1, 2, 3])], [torch.tensor([4, 5, 6])], [torch.tensor([7, 8, 9])]],
            phon_probs=[[torch.tensor([0.1, 0.2, 0.3])], [torch.tensor([0.4, 0.5, 0.6])], [torch.tensor([0.7, 0.8, 0.9])]]
        )
