...
sweep_config = {
    ...
    "parameters": {
        ...
        "d_model": {"values": [128,64,256]},
        "d_embedding": {"values": [1,2,4]},
        "num_epochs": {"values": [100]},
        "learning_rate": {"values": [0.0001,0.0005,0.001]},
        "num_phon_enc_layers": {"values": [8,16]},
        "num_phon_dec_layers": {"values": [2,4]},
        "num_mixing_enc_layers": {"values": [2,4]},
        "nhead": {"values": [8,16]},
    },
}
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(
        sweep=sweep_config, project=os.environ.get("WAND_PROJECT", "BRIDGE")
    )
    wandb.agent(
        sweep_id, function=pretrain, project=os.environ.get("WAND_PROJECT", "BRIDGE")
    )
