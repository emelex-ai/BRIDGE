...
class Model(torch.nn.Module):
    def __init__(self,
        ...
        self.orth_position_embedding = torch.nn.Embedding(max_seq_len, d_model)  # GE May 31, 2023
        # Embedding for phonology
        self.phonology_embedding = torch.nn.Embedding(phon_vocab_size, d_model)
        self.phon_position_embedding = torch.nn.Embedding(max_seq_len, d_model)  # GE May 31, 2023
    ...
    def embed_tokens(self, tokens, mode='o'):
        ...
        if mode == 'o':
           ...
           return self.orthography_embedding(tokens) + self.orth_position_embedding.weight[None,:tokens.shape[1]] # changed pos encoding variable name May 31
        ...
        # This is where we need to average the phonological embedding vectors
           ...
           #return output_embedding + self.position_embedding.weight[None,:len(tokens[0])]    # GE
           return output_embedding + self.phon_position_embedding.weight[None,:len(tokens[0])]  # GE May 31, 2023
