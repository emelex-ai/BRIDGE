...
into_reading_text_table = into_reading_sequentially_arranged %>% 
  ...
  summarise(n = n(),
            ...
            grade = first(grade)) %>% # grade_as_alpha is the sequential version of grade: k = a, 1 = b, 2 = c
...
for (PROPORTION in unique(finetuning_ids$proportion_background)){
  ...
  target_id = finetuning_ids %>% 
    filter(proportion_background == PROPORTION)
  
  stopifnot(nrow(target_id) == 2) # should only return two entries (one for each sampling condition)
  stopifnot(length(unique(target_id$sampling_condition)) == 2) # has to be two different levels of condition
  
  #PROPORTION = pull(target_id, proportion_background) # proportion of background vocabulary (old code)
  #CONDITION = pull(target_id, sampling_condition)
  
  frequency_weighted_list = list()
  length_weighted_list = list()
  ...
  for (text_index in sort(into_reading_text_table$text_index_in_program)){
    ...
    frequency_weighted_reconstructed = list()
    length_weighted_reconstructed = list()
    
    for(id in target_id$finetuning_id){
    
      CONDITION = target_id %>% 
        filter(finetuning_id == id) %>% 
      ...
      for (iter in seq(N)){
          ...
          if (CONDITION == "frequency_weighted"){
            ...
            target_text_with_sampled_background = target %>% 
              ...
              select(word, word_index_in_text, doc_id, background_or_not, grade) %>% 
              rbind(background_vocab) %>% # using more restrictive rbind() for control
              arrange(word_index_in_text) %>% 
              left_join(target_metadata) %>% 
              mutate(sampling_condition = CONDITION,
                     iteration = iter)
          
            frequency_weighted_reconstructed[[as.character(iter)]] = target_text_with_sampled_background
          ...
          if (CONDITION == "length_weighted"){
            ...
            length_weighted_reconstructed[[as.character(iter)]] = target_text_with_sampled_background
      ...
      frequency_weighted_list[[text_index]] = list_rbind(frequency_weighted_reconstructed)
      length_weighted_list[[text_index]] = list_rbind(length_weighted_reconstructed)
  ...
  frequency_weighted_id = target_id %>% 
    filter(sampling_condition == "frequency_weighted") %>% 
    pull(finetuning_id)
  
  length_weighted_id = target_id %>% 
    filter(sampling_condition == "length_weighted") %>% 
    pull(finetuning_id)
  
  filename_frequency_weighted = paste(writepath, "/", frequency_weighted_id, ".csv", sep = "")
  filename_length_weighted = paste(writepath, "/", length_weighted_id, ".csv", sep = "")

  
  frequency_weighted_list %>% 
    list_rbind() %>% 
    ...
    write_csv(filename_frequency_weighted)
  
  
  length_weighted_list %>% 
    list_rbind() %>% 
    rownames_to_column(var = "word_order_in_dataset") %>% 
    mutate(count = 1,
           proportion_background = PROPORTION) %>% 
    select(word_order_in_dataset, word_raw = word, count, background_or_not, 
           text_name, doc_id, text_index_in_program, grade, grade_as_alpha, 
           unit_within_grade, text_number_within_unit, word_index_in_text, num_words_in_text = n, everything()) %>% # everything() is catchall 
    write_csv(filename_length_weighted)
    
  end = Sys.time()
  elapsed = end-start
  print(paste(PROPORTION, "done..."))
  print(paste("Loop for proportion", PROPORTION, "took", round(elapsed[[1]]/60, digits = 2), "minutes"))
}
...
# Finetuning dataset: words only version
...
for (PROPORTION in unique(finetuning_ids$proportion_background)){
    ...
    stopifnot(all.equal(sort(tmp_a$word_order_in_dataset), sort(tmp_b$word_order_in_dataset)))
...
redundant_finetuning_id = finetuning_ids %>% 
  ...
  pull(finetuning_id)
file.remove(paste(writepath2, redundant_finetuning_id, ".csv", sep = ""))
