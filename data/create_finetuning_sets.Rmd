...
for (FINETUNING_ID in finetuning_ids$finetuning_id){
    ...
    for (i in seq(N)){
      ...
      into_reading_sample_[[as.character(i)]] = into_reading_sample %>% 
        mutate(num_times_through_text = i)
    }
  
    
    # final renames, arranges, etc:
    into_reading_sample_ %>% 
      list_rbind() %>%
      ungroup() %>%  # make sure the df is ungrouped
      mutate(background_or_not = case_when(word_index_sampled ~ "background_vocabulary",
                                           !word_index_sampled ~ "not_background_vocabulary")) %>% 
      rename(word_raw = word, sampling_condition = condition) %>%
      arrange(grade_as_alpha, unit_within_grade, text_number_within_unit, num_times_through_text, word_index_in_text) %>%
      mutate(word_index_in_dataset = seq_len(n())) %>% 
      select(-word_sampled, -word_index_sampled) %>% 
      select(word_raw, word_index_in_dataset, doc_id, text_name, num_times_through_text, grade_as_alpha, grade, text_number_within_unit, everything()) %>% 
...
```{r}
cat("##############################################\n")
cat(paste("   Testing for datasets in:", writepath, "\n"))
cat("   ------------------------------------------\n")
...
for (i in seq(N)){
  row_indices_in_program_list[[as.character(i)]] = into_reading_sequentially_arranged %>% 
    mutate(num_times_through_text = i) #%>% 
    #select(word_index_in_program)
}

  

row_indices_in_program_and_dataset = row_indices_in_program_list %>% 
  list_rbind() %>% 
  ungroup() %>% 
  arrange(grade_as_alpha, unit_within_grade, text_number_within_unit, num_times_through_text, word_index_in_text) %>%
  mutate(target_word_index_in_dataset = seq_len(n())) %>% 
  select(target_word_index_in_program = word_index_in_program, target_word_index_in_dataset)
...
for (filename in filenames){
  ...
  cat(paste("Number total rows", num_rows, "\n"))
  cat(paste("Number unique words:", num_unique_words), "\n")
  
  all_ = all(c(num_rows, max_index_, num_total_words) == num_rows)
  
  cat(paste("All equivalent: ", all_, "\n"))
 
  num_unique_words_all = c(num_unique_words_all, num_unique_words)
  all_values_equal = c(all_values_equal, all_)
  
  row_indices_in_program = cbind(row_indices_in_program_and_dataset$target_word_index_in_program, tmp$word_index_in_program)
  row_indices_in_dataset = cbind(row_indices_in_program_and_dataset$target_word_index_in_dataset, tmp$word_index_in_dataset)
...
for (PROPORTION in unique(finetuning_ids$proportion_background)){
  ...
  cat("##########################################\n")
  cat(paste("Testing proportion:", PROPORTION, "\n"))
    ...
    cat("  -----------------------------------\n")
    cat(paste(">> Proportion", PROPORTION, "passes\n"))
    cat("  -----------------------------------\n")
