...
for (id in maintrain_ids$maintrain_id){
  start = Sys.time()
    ...
    for (iter in seq(N)){
        ...
        if (id %in% frequency_weighted_condition_ids){
          ...
          background_vocab = ewfg_ %>% 
            ...
            select(word, word_index_in_text, doc_id)
  ...
  into_reading_list %>% 
    ...
    mutate(count = 1) %>% 
    select(word_raw = word, count, everything()) %>% 
    rename(num_words_in_text = n) %>% 
    write_csv(filename)
...
filenames = list.files(path = "finetuning/1_time/", pattern = "*.csv", full.names = T)

for (filename in filenames){
  
  ID = str_extract(filename, "(?<=maintraining/).*?(?=\\.csv)")
  
  outpath = paste("tmp/by_word/", ID, "_by_word.csv", sep = "")
...
tmp = read_csv("maintraining/by_word/1_by_word.csv")
```
...
filenames = list.files(path = "tmp", full.names = T, pattern = ".csv$")
...
for (filename in filenames){
  ...
  #print(paste("Number unique row numbers:", length(unique(tmp$word_order_in_dataset))))
  #print(paste("Max row numbers:", max_index))
