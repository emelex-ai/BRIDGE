...
if CONTINUE:
  ...
  model = Model(orth_vocab_size=len(ds.character_tokenizer), 
                ...
                nhead=chkpt['nhead'],
                num_layers=2)  # GE: should be retrieved from checkpoint file
  model.load_state_dict(chkpt['model'])
  opt = torch.optim.AdamW(model.parameters(), learning_rate)
  opt.load_state_dict(chkpt['optimizer'])
else:
  model = Model(len(ds.character_tokenizer), len(ds.phonology_tokenizer), d_model=d_model, nhead=nhead, num_layers=num_layers) # added num_layers
  opt = torch.optim.AdamW(model.parameters(), learning_rate)
...
for epoch in pbar:
    ...
    for step, batch_slice in enumerate(train_dataset_slices):
        ...
        print(f"{epoch}/{step}: B_phon shape:", A_phon.size(), "\n")  # GE: changed
